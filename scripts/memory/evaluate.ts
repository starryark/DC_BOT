...
import { EVALUATOR_SCHEMA_VERSION, parseThresholdDocument } from '../../evals/memory/contracts'
...
Options:
  ...
  [--retrieval-mode lexical|vector|graph] Candidate mode for retrieval qualification
...
interface ParsedArgs {
  ...
  output?: string
  thresholds?: string
  retrievalMode?: 'lexical' | 'vector' | 'graph'
  keepRunRoot: boolean
  help: boolean
}
...
async function run(args: ParsedArgs, repoRoot: string): Promise<number> {
  if (args.retrievalMode && args.retrievalMode !== 'lexical') {
    process.stderr.write(`${JSON.stringify({ status: 'unsupported', requestedMode: args.retrievalMode, reason: `${args.retrievalMode} retrieval is not implemented or advertised` })}\n`)
    return EXIT.INVALID_CONFIG
  }
  if (args.retrievalMode && args.suite !== 'multilingual-v1') {
    process.stderr.write(`${JSON.stringify({ status: 'invalid', message: '--retrieval-mode is only valid with --suite multilingual-v1' })}\n`)
    return EXIT.INVALID_CONFIG
  }
  ...
  if (args.thresholds) {
    ...
    try {
      thresholds = parseThresholdDocument(JSON.parse(readFileSync(thresholdPath, 'utf8')), { datasetVersion: dataset.datasetVersion, datasetDigest, evaluatorSchemaVersion: EVALUATOR_SCHEMA_VERSION })
    }
    catch (error) {
      process.stderr.write(`${JSON.stringify({ status: 'invalid', message: messageOf(error) })}\n`)
      return EXIT.INVALID_CONFIG
  ...
  const report = buildReport({ dataset, datasetDigest, seed: args.seed, commitSha, dirtyWorktree: gitDirty(repoRoot), platform: process.platform, nodeVersion: process.version, arch: process.arch, generatedAt: new Date().toISOString(), results, expectedScenarioTotal: scenarios.length, thresholds })
...
function parseArgs(values: readonly string[]): ParsedArgs {
  ...
  for (let index = 0; index < values.length; index++) {
    ...
    else if (value === '--retrieval-mode') {
      const mode = values[++index]
      if (mode !== 'lexical' && mode !== 'vector' && mode !== 'graph')
        throw new Error(`--retrieval-mode must be lexical, vector, or graph, got ${mode ?? '(missing)'}`)
