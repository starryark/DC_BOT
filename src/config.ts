...
export interface CharacterConfig {
   ...
   * bot's working directory (the project root). Empty disables the
...
export function config(): AppConfig {
  ...
  const result: AppConfig = {
    ...
    character: {
      // The bundled card is <repo>/characters/Makise Kurisu/card.json, and the registry
      // joins `<root>/<id>/card.json` beneath the local characters directory.
      // Use `??` (not `||`) so
      // CHARACTER_PATH= can deliberately turn the character runtime off.
      root: env.CHARACTER_PATH ?? './characters',
