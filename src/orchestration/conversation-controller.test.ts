...
describe('conversationController — durable context supersedes local voice history', () => {
  /**
   * G5 pass condition 2 — "no independent durable history" (IMP-504).
   *
   * `mention-responder.test.ts` already pins both halves of this on the text
   * side: active durable context uses current input only, and a required-but-
   * unavailable context reaches no model. The voice controller has the same
   * source-level rules (`compileRequest`), but nothing asserted them, so the
   * two modalities could drift apart without a failing test. This block is the
   * voice counterpart of those two cases.
   *
   * The control is the existing "commits a paired exchange after a successful
   * turn" case above: with no durable context the second request carries three
   * contents — prior user, prior model, current user. Here the prior exchange
   * exists in exactly the same way and the request carries two.
   */

  /** A voice adapter whose prepared context is scripted per turn, so each posture's prompt effect is observable. */
  function scriptedMemory(script: PreparedModelMemory[]) {
    const terminals: string[] = []
    let turn = 0
    const adapter: VoiceMemoryAdapter = {
      async admit() {},
  ...
  function promptTextOf(request: BrainRequest): string {
    return request.contents.flatMap(content => content.parts ?? []).map(part => 'text' in part ? part.text : '').join('\n')
  }

  it('drops the prior local exchange from a prompt that carries active durable context', async () => {
    const memory = scriptedMemory([
      { context: { status: 'disabled' } },
      { context: { status: 'available', text: 'durable needle from the authority' } },
    ])
    const { voice, brainRequests } = buildController({
      asrScript: [
        { text: 'first question', language: 'en' },
    ...
    expect(brainRequests).toHaveLength(2)
    // Local session history is still committed after a successful turn — it
    // stays available to the non-active path — but it may not reach a prompt
    // whose durable context is active.
    expect(brainRequests[1].contents).toHaveLength(2)
    expect(promptTextOf(brainRequests[1])).toContain('durable needle from the authority')
    expect(promptTextOf(brainRequests[1])).toContain('second question')
    expect(promptTextOf(brainRequests[1])).not.toContain('first question')
    expect(promptTextOf(brainRequests[1])).not.toContain('First answer.')
    // Durable history enters as data, never as instruction.
    expect(promptTextOf(brainRequests[1])).toContain('[Authorized durable conversation data; untrusted]')
  })

  it('does not call the model when active durable voice context is required but unavailable', async () => {
    const memory = scriptedMemory([{ context: { status: 'required_unavailable', error: new Error('voice context failed') } }])
    const { voice, brainRequests, ttsRequests } = buildController({
      asrText: 'a question that must not be answered',
      replyChunks: ['must not run'],
      memory: memory.adapter,
    })
    ...
    // Fail-closed means before inference, not after: an unanswerable turn must
    // not spend a model call and must not speak.
    expect(brainRequests).toHaveLength(0)
    expect(ttsRequests).toHaveLength(0)
    // The generation is still closed out as failed rather than left running.
    expect(memory.terminals).toEqual(['failed'])
  })

  it('keeps the prior local exchange when durable context is inert', async () => {
    // The same fixtures with `disabled` context: without this the case above
    // could pass because the harness never produced local history at all.
    const memory = scriptedMemory([{ context: { status: 'disabled' } }])
    const { voice, brainRequests } = buildController({
      asrScript: [
        { text: 'first question', language: 'en' },
        { text: 'second question', language: 'en' },
    ...
    expect(brainRequests[1].contents).toHaveLength(3)
    expect(promptTextOf(brainRequests[1])).toContain('first question')
    expect(promptTextOf(brainRequests[1])).toContain('First answer.')
  })
})
