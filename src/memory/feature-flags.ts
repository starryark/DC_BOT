...
const M1_GATED: Partial<Record<keyof MemoryFeatureFlags, string>> = {
  // Named separately and precisely because IMP-607 has already run: a reason
  // saying "requires an accepted benchmark (IMP-607)" reads, after the fact, as
  // though the benchmark that exists satisfies it. It does not — IMP-607
  // measured lexical retrieval, and its disposition is `measured_not_evaluated`.
  vectorRetrieval: 'ADR-011: vector retrieval requires an accepted vector retrieval benchmark; the IMP-607 lexical benchmark does not evaluate it',
  relationshipHypotheses: 'artifact 22 EV-012: relationship/graph work requires benchmark evidence for graph storage, which no implementation yet produces',
