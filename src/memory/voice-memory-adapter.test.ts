...
import type { RecentContextRequest, RecentContextResult } from './context-authority'
...
import { classifyVoicePreparationFailure } from '../../evals/memory/performance/fixtures/voice'
...
/**
 * The durable context deadline `prepareGeneration` puts around
 * `assembleRecent`.
 *
 * The bound is production availability policy: a voice turn that cannot get its
 * durable context inside it fails closed rather than speaking without memory.
 * Nothing executed that bound before — the adapter tests all resolved context
 * immediately — so the benchmark's intermittent
 * `active-memory-terminal-state` failure could not be attributed to it or ruled
 * out from source alone.
 *
 * The context authority is replaced on the runtime the adapter already holds,
 * which is the seam the shadow-mode test above uses. The deadline value itself
 * is not changed by these tests.
 */
describe('voice memory adapter durable context deadline', () => {
  /** Replace the runtime's context assembler, keeping the real one reachable. */
  function interceptContext(
    runtime: ReturnType<typeof runtimeFor>,
    intercept: (request: RecentContextRequest, real: (request: RecentContextRequest) => Promise<RecentContextResult>) => Promise<RecentContextResult>,
  ): void {
    const context = runtime.context
    if (!context)
      throw new Error('active runtime must expose a context authority')
    const real = context.assembleRecent.bind(context)
    context.assembleRecent = request => intercept(request, real)
  }

  it('admits durable context that resolves inside the deadline', async () => {
    const runtime = runtimeFor('active', 'airi-voice-deadline-ok-')
    const adapter = createVoiceMemoryAdapter({ runtime, characterId, modelRef: 'test/model' })
    const earlier = voiceEvent(20)
    await adapter.admit(earlier, 'earlier utterance')
    await adapter.prepareGeneration(earlier.turnId, [earlier])
    await adapter.completeGeneration(earlier.turnId)

    const current = voiceEvent(21)
    await adapter.admit(current, 'current utterance')
    // Resolved on a macrotask rather than synchronously, so the race against a
    // live timer is genuinely raced. A fixed sleep would put a machine-speed
    // assumption into the assertion instead.
    interceptContext(runtime, async (request, real) => {
      await new Promise(resolve => setTimeout(resolve, 0))
      return real(request)
    })

    const prepared = await adapter.prepareGeneration(current.turnId, [current])
  ...
  it('fails closed and begins no generation when durable context crosses the deadline', async () => {
    const runtime = runtimeFor('active', 'airi-voice-deadline-late-')
    const adapter = createVoiceMemoryAdapter({ runtime, characterId, modelRef: 'test/model' })
    const event = voiceEvent(22)
    await adapter.admit(event, 'utterance')
    ...
    // The held context resolves and never rejects: a value that arrives after
    // the deadline must not leave an unhandled rejection behind when it lands.
    let lateAssembly: Promise<RecentContextResult> | undefined
    interceptContext(runtime, (request, real) => {
      lateAssembly = held.then(() => real(request))
      return lateAssembly
    })
    const beginGeneration = vi.spyOn(runtime.trace!, 'beginGeneration')

    await expect(adapter.prepareGeneration(event.turnId, [event])).rejects.toThrow(/context deadline exceeded/i)
    expect(beginGeneration).not.toHaveBeenCalled()

    // Let the late context land. The race was already lost, so nothing
    // downstream may act on the value that arrives now.
    releaseLate()
    await lateAssembly
    expect(beginGeneration).not.toHaveBeenCalled()
    await runtime.close()
  ...
  it('is classified by the benchmark as a deadline, not as a generic preparation failure', async () => {
    // NOTICE:
    // The benchmark classifies this failure by matching the message production
    // throws, because production raises one plain `Error` and adding a typed
    // diagnostic to it solely so the benchmark could read it is the production
    // coupling the eval decorator exists to avoid.
    //
    // That match is the thing that can rot silently: a reworded message would
    // downgrade every future deadline to `context-preparation-failed` and the
    // benchmark would keep publishing confidently wrong evidence. Asserting it
    // against the real thrown error is what keeps the two layers honest.
    //
    // Removal condition: production exposes a typed context-deadline failure.
    const runtime = runtimeFor('active', 'airi-voice-deadline-classify-')
    const adapter = createVoiceMemoryAdapter({ runtime, characterId, modelRef: 'test/model' })
    const event = voiceEvent(23)
    await adapter.admit(event, 'utterance')
    ...
    const deadlineError = await adapter.prepareGeneration(event.turnId, [event]).then(() => undefined, (error: unknown) => error)
    expect(classifyVoicePreparationFailure(deadlineError)).toBe('context-deadline-exceeded')

    releaseLate()
    await lateAssembly
    await runtime.close()

    // A durable authority that is simply gone is a different failure, and must
    // not be reported as a latency problem.
    const authorityError = await adapter.prepareGeneration(event.turnId, [event]).then(() => undefined, (error: unknown) => error)
    expect(classifyVoicePreparationFailure(authorityError)).toBe('context-preparation-failed')
  })
})
