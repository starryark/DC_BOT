...
describe('capability gating (ADR-001, ADR-011)', () => {
  ...
  // ROOT CAUSE:
  //
  // Both reasons read 'ADR-011: requires an accepted retrieval benchmark
  // (IMP-607)'. IMP-607 then ran, over a lexical dataset, and produced a
  // result — leaving two gates whose stated unlock condition names a task that
  // had visibly completed. Nothing in the string distinguished "the benchmark
  // for this capability" from "a benchmark", so the lexical evidence read as
  // though it discharged both gates.
  //
  // We fixed this by naming the missing evidence per capability, and by moving
  // graph onto its own decision (artifact 22 EV-012) since canonical ADR-011
  // does not place graph in the retrieval pipeline at all.
  it('does not let the lexical IMP-607 benchmark read as the unlock for vector or graph', () => {
    const vector = M1_FORBIDDEN_CAPABILITIES.vector_search!
    const graph = M1_FORBIDDEN_CAPABILITIES.graph_search!

    // Vector may cite IMP-607 only to disclaim it.
    expect(vector).toMatch(/vector retrieval benchmark/)
    expect(vector).toMatch(/IMP-607 lexical benchmark does not evaluate/)

    // Graph is held by a different decision, so IMP-607 is not even in scope.
    expect(graph).not.toMatch(/IMP-607/)
    expect(graph).toMatch(/EV-012/)

    // Distinct unlock conditions: discharging one must not read as discharging
    // the other.
    expect(vector).not.toEqual(graph)
  })
