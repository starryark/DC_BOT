...
import {
  ...
  serializeLayeredContextManifest,
...
describe('canonical layered context manifest', () => {
  const manifest = {
    formatVersion: 1,
    queryDigest: 'a'.repeat(64),
    logicalRoomVersion: 2,
    bindingRevision: 3,
    authorizationRevision: 4,
    maxItems: 12,
    maxCharacters: 2_000,
    requestedLayers: ['semantic'],
    appliedModes: ['exact'],
    selected: [],
    omittedLayers: [],
    truncated: false,
  ...
  it('serializes only a SHA-256 query digest and rejects content-bearing header fields', () => {
    expect(serializeLayeredContextManifest(manifest)).toContain(`"queryDigest":"${'a'.repeat(64)}"`)
    expect(() => serializeLayeredContextManifest({ ...manifest, queryDigest: 'private query text' } as never)).toThrow(/SHA-256/)
    expect(() => serializeLayeredContextManifest({ ...manifest, exactPredicateDigest: 'private predicate' } as never)).toThrow(/SHA-256/)
    expect(() => serializeLayeredContextManifest({ ...manifest, analyzerIdentity: 'private analyzer detail' } as never)).toThrow(/analyzer identity/)
    expect(() => serializeLayeredContextManifest({ ...manifest, formatVersion: 2 } as never)).toThrow(/unknown layered context manifest format/)
  })
})
