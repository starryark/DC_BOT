 ...
 *
 * Each reason names the evidence that is missing, not merely the benchmark task
 * that would produce it. The distinction is load-bearing. IMP-607 ran and
 * produced a result, so a reason reading "requires an accepted retrieval
 * benchmark (IMP-607)" invites the reading that the gate is now satisfied —
 * when IMP-607 measured *lexical* retrieval only, was never able to measure
 * anything else (its qualifier refuses any mode but `lexical`), and reached
 * `measured_not_evaluated` rather than acceptance in any case.
 *
 * Vector and graph are held by different decisions and are deliberately not
 * collapsed into one reason: satisfying either unlocks only itself.
 */
export const M1_FORBIDDEN_CAPABILITIES: Readonly<Partial<Record<Capability, string>>> = Object.freeze({
  // ADR-011 puts vector last in the retrieval pipeline and gates it there.
  // D-DEF-002 additionally defers the vector store product choice to M2/M3, so
  // there is no backend to benchmark yet.
  vector_search: 'ADR-011: requires an accepted vector retrieval benchmark; the IMP-607 lexical benchmark does not evaluate vector retrieval and cannot unlock it',
  // Canonical ADR-011 does not place graph in the retrieval pipeline at all.
  // The gate is artifact 22 EV-012, which requires benchmark evidence before
  // graph storage is adopted — and no graph store exists to produce it.
  graph_search: 'artifact 22 EV-012: requires benchmark evidence for graph storage, which no implementation or benchmark yet produces; not covered by ADR-011 retrieval evidence',
