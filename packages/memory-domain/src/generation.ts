...
import type { MemoryLayer } from './memory-records'
import type { ExtractionMethod, ProvenanceSource } from './provenance'

import { MemoryError } from './errors'
import { asTimestamp, isValidId } from './ids'
...
export interface SnapshotEvidence {
  ...
  /** G6 layered selection evidence, present only when derived retrieval influenced the prompt. */
  layeredContextManifest?: LayeredContextManifest
  /** Canonical digest of {@link layeredContextManifest}. */
  layeredContextManifestHash?: string
...
/** Content-free evidence identifying one G6-selected context record. */
export type LayeredContextManifestItem
  = | { readonly layer: 'recent', readonly reason: 'recent', readonly source: SnapshotContextItem }
    | {
      readonly layer: 'summary' | 'semantic' | 'episodic'
      readonly recordId: string
      readonly reason: 'exact' | 'lexical' | 'current-summary'
...
/** Reproducible, content-free trace of a G6 layered selection. */
export interface LayeredContextManifest {
  readonly formatVersion: 1
  readonly queryDigest: string
  readonly exactPredicateDigest?: string
  readonly logicalRoomVersion: number
  readonly bindingRevision: number
  ...
  readonly maxItems: number
  readonly maxCharacters: number
  readonly requestedLayers: readonly Extract<MemoryLayer, 'recent' | 'summary' | 'semantic' | 'episodic'>[]
  readonly appliedModes: readonly ('exact' | 'lexical' | 'current-summary')[]
  readonly analyzerIdentity?: 'sqlite-fts5-cjk-trigram-v1' | 'sqlite-fts5-latin-unicode61-v1'
  readonly selected: readonly LayeredContextManifestItem[]
  readonly omittedLayers: readonly Extract<MemoryLayer, 'recent' | 'summary' | 'semantic' | 'episodic'>[]
  readonly truncated: boolean
}

/** Serializes a layered manifest with a stable key and item order. */
export function serializeLayeredContextManifest(manifest: LayeredContextManifest): string {
  if (manifest.formatVersion !== 1)
    throw new MemoryError('POLICY_VIOLATION', 'unknown layered context manifest format')
  if (!/^[a-f\d]{64}$/u.test(manifest.queryDigest))
    throw new MemoryError('POLICY_VIOLATION', 'layered context queryDigest must be a SHA-256 hex digest')
  if (manifest.exactPredicateDigest && !/^[a-f\d]{64}$/u.test(manifest.exactPredicateDigest))
    throw new MemoryError('POLICY_VIOLATION', 'layered context exactPredicateDigest must be a SHA-256 hex digest')
  if (manifest.asOf)
    asTimestamp(manifest.asOf)
  if (manifest.analyzerIdentity && manifest.analyzerIdentity !== 'sqlite-fts5-cjk-trigram-v1' && manifest.analyzerIdentity !== 'sqlite-fts5-latin-unicode61-v1')
    throw new MemoryError('POLICY_VIOLATION', 'unknown layered context analyzer identity')
  for (const item of manifest.selected) {
    if (item.layer === 'recent')
      continue
    if (!isValidId(item.recordId) || item.sourceEventIds.some(eventId => !isValidId(eventId)))
      throw new MemoryError('POLICY_VIOLATION', 'layered context manifest identifiers must be bounded opaque tokens')
    asTimestamp(item.validFrom)
    asTimestamp(item.recordedAt)
  }
  return JSON.stringify({
    formatVersion: manifest.formatVersion,
    queryDigest: manifest.queryDigest,
    ...(manifest.exactPredicateDigest ? { exactPredicateDigest: manifest.exactPredicateDigest } : {}),
    ...
    authorizationRevision: natural(manifest.authorizationRevision, 'authorizationRevision'),
    ...(manifest.asOf ? { asOf: manifest.asOf } : {}),
    maxItems: natural(manifest.maxItems, 'maxItems'),
    maxCharacters: natural(manifest.maxCharacters, 'maxCharacters'),
    requestedLayers: [...manifest.requestedLayers],
    appliedModes: [...manifest.appliedModes],
    ...(manifest.analyzerIdentity ? { analyzerIdentity: manifest.analyzerIdentity } : {}),
    selected: manifest.selected.map(item => item.layer === 'recent'
      ? { layer: item.layer, reason: item.reason, source: item.source }
      : { layer: item.layer, recordId: item.recordId, reason: item.reason, validFrom: item.validFrom, recordedAt: item.recordedAt, provenanceSource: item.provenanceSource, extractionMethod: item.extractionMethod, sourceEventIds: [...item.sourceEventIds], ...(item.rank == null ? {} : { rank: natural(item.rank, 'rank') }) }),
    omittedLayers: [...manifest.omittedLayers],
