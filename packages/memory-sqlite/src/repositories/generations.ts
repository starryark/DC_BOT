...
import type { CharacterId, DeliveryState, GenerationAttempt, GenerationId, GenerationState, LayeredContextManifest, LogicalRoomId, SnapshotContextItem, SnapshotContextManifest, Timestamp } from '@proj-airi/memory-domain'

import { createHash, randomUUID } from 'node:crypto'
import { asCharacterId, asDeliveryId, asEventId, asGenerationId, asLogicalRoomId, asRequestId, asSegmentId, asTimestamp, digestSnapshotContextManifest, MemoryError, serializeLayeredContextManifest, transitionGeneration } from '@proj-airi/memory-domain'
...
function reconstruct(db: DatabaseSync, row: Row): GenerationAttempt {
  ...
  const layeredRow = db.prepare('SELECT manifest_json,manifest_hash FROM generation_layered_context_manifests WHERE generation_id=?').get(row.generation_id) as { manifest_json: string, manifest_hash: string } | undefined
  const layeredContextManifest = layeredRow ? JSON.parse(layeredRow.manifest_json) as LayeredContextManifest : undefined
  if (layeredRow && layeredContextManifest && createHash('sha256').update(serializeLayeredContextManifest(layeredContextManifest)).digest('hex') !== layeredRow.manifest_hash)
    throw new MemoryError('PERSISTENCE_FAILED', 'layered context manifest digest does not reconstruct')
  return {
    ...
    evidence: { observedRoomVersion: Number(row.observed_room_version), observedEventIds, contextManifestHash: String(row.context_manifest_hash), contextManifest, ...(layeredContextManifest && layeredRow ? { layeredContextManifest, layeredContextManifestHash: layeredRow.manifest_hash } : {}), observedBindingVersion: Number(row.observed_binding_version), capturedAt: asTimestamp(String(row.captured_at)) },
...
export class GenerationRepository {
  ...
  create(attempt: GenerationAttempt): CreateGenerationResult {
    ...
    const layeredContextManifestHash = attempt.evidence.layeredContextManifest
      ? createHash('sha256').update(serializeLayeredContextManifest(attempt.evidence.layeredContextManifest)).digest('hex')
      : undefined
    const canonicalAttempt = { ...attempt, evidence: { ...attempt.evidence, contextManifestHash: digestSnapshotContextManifest(attempt.evidence.contextManifest), ...(layeredContextManifestHash ? { layeredContextManifestHash } : {}) } }
    ...
    try {
      ...
      for (const item of canonicalAttempt.evidence.layeredContextManifest?.selected ?? []) {
        if (item.layer === 'recent') {
          if (!manifest.items.some(recent => JSON.stringify(recent) === JSON.stringify(item.source)))
            throw new MemoryError('POLICY_VIOLATION', 'layered manifest recent item is absent from the durable recent selection')
          continue
        }
        let inScope: unknown
        if (item.layer === 'summary')
          inScope = this.db.prepare('SELECT 1 FROM summary_repository_records WHERE summary_id=? AND logical_room_id=?').get(item.recordId, attempt.logicalRoomId)
        else if (item.layer === 'semantic')
          inScope = this.db.prepare('SELECT 1 FROM semantic_fact_repository_records WHERE fact_id=? AND scope_kind=\'logical_room\' AND scope_id=?').get(item.recordId, attempt.logicalRoomId)
        else
          inScope = this.db.prepare('SELECT 1 FROM episodic_repository_records WHERE episodic_id=? AND logical_room_id=?').get(item.recordId, attempt.logicalRoomId)
        if (!inScope)
          throw new MemoryError('SCOPE_LEAK_DETECTED', 'layered manifest record is outside the generation room')
      }
      ...
      if (canonicalAttempt.evidence.layeredContextManifest && canonicalAttempt.evidence.layeredContextManifestHash) {
        const layered = canonicalAttempt.evidence.layeredContextManifest
        if (layered.logicalRoomVersion !== manifest.logicalRoomVersion || layered.bindingRevision !== manifest.bindingRevision)
          throw new MemoryError('POLICY_VIOLATION', 'layered manifest scope evidence conflicts with the recent-context manifest')
        this.db.prepare('INSERT INTO generation_layered_context_manifests VALUES (?,?,?,?)').run(attempt.generationId, layered.formatVersion, serializeLayeredContextManifest(layered), canonicalAttempt.evidence.layeredContextManifestHash)
      }
