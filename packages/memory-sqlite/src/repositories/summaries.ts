...
export class SummaryRepository {
  ...
  /** Lists only summaries eligible for current retrieval while preserving {@link list} as history. */
  listActive(logicalRoomId: SummaryRecord['logicalRoomId']): readonly SummaryRecord[] {
    try { return (this.db.prepare('SELECT * FROM summary_repository_records WHERE logical_room_id=? AND tombstoned_by IS NULL AND stale=0 AND superseded_by IS NULL AND valid_until IS NULL ORDER BY recorded_at,summary_id').all(logicalRoomId) as SqlRow[]).map(row => this.reconstruct(row)) }
    catch (error) { persistence('SQLite active-summary listing failed', error) }
  }

  /** Lists summaries eligible at a historical instant using half-open validity. */
  listAsOf(logicalRoomId: SummaryRecord['logicalRoomId'], at: SummaryRecord['validity']['validFrom']): readonly SummaryRecord[] {
    try { return (this.db.prepare('SELECT * FROM summary_repository_records WHERE logical_room_id=? AND tombstoned_by IS NULL AND stale=0 AND valid_from<=? AND (valid_until IS NULL OR valid_until>?) ORDER BY recorded_at,summary_id').all(logicalRoomId, at, at) as SqlRow[]).map(row => this.reconstruct(row)) }
    catch (error) { persistence('SQLite as-of summary listing failed', error) }
  }
