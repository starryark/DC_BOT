...
export class CorrectionRepository {
  ...
  correct(correctionId: string, input: Omit<CorrectionInput, 'previous'> & { previousFactId: FactId }): CorrectionResult & { deduplicated: boolean } {
    ...
    try {
      ...
      // No summary dependency is inferred here: the current schema persists no
      // semantic-fact-to-summary edge. This shared vocabulary can act when such
      // a dependency is represented by a future reviewed schema.
      void new DerivedInvalidationPlanner(this.db).plan({ kind: 'correction', correctionId, supersededFactId: previous.factId, replacementFactId: fact.factId })
      new DerivedRepairQueue(this.db).enqueueLexicalRepair({ jobId: `lexical:${correctionId}`, dedupeKey: `correction:${correctionId}`, reason: 'correction', policyVersion: 'lexical-v1', availableAt: input.recordedAt, createdAt: input.recordedAt, maxAttempts: 3 })
