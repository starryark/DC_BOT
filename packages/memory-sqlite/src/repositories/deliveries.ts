...
export class DeliveryRepository {
  ...
  /** Attempts a prior process left `pending` or `delivering`; IMP-406 classifies these as crash-ambiguous at startup. */
  inFlight(): readonly DeliveryAttempt[] { try { return (this.db.prepare(`SELECT * FROM delivery_attempt_records WHERE current_state IN ('pending','delivering') ORDER BY started_at,delivery_id`).all() as Row[]).map(attempt) } catch (error) { persistence('SQLite in-flight delivery lookup failed', error) } }

  /** Content-free count of attempts in one state; backs the operator-review health signal (e.g. `abandoned`). */
  countByState(state: DeliveryState): number { try { return Number((this.db.prepare('SELECT COUNT(*) count FROM delivery_attempt_records WHERE current_state=?').get(state) as { count: number }).count) } catch (error) { persistence('SQLite delivery state count failed', error) } }
