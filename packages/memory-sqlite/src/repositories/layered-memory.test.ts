...
describe('IMP-206 layered memory repositories', () => {
  it('creates and exactly reconstructs every layer with deterministic summary ordering and complete lineage', () => {
    ...
    db.prepare('UPDATE summary_repository_records SET stale=1 WHERE summary_id=?').run('summary-a'); expect(summaries.list(roomId).map(item => item.summaryId)).toEqual(['summary-a', 'summary-b']); expect(summaries.listActive(roomId).map(item => item.summaryId)).toEqual(['summary-b'])
  ...
  it('atomically creates a multi-step correction chain and exact retries add no rows', () => {
    ...
    expect(db.prepare("SELECT job_type,dedupe_key FROM worker_jobs ORDER BY job_id").all()).toEqual([{ job_type: 'derived_search_rebuild', dedupe_key: 'correction:correction-a' }, { job_type: 'derived_search_rebuild', dedupe_key: 'correction:correction-b' }])
  })
  ...
  it('rolls back base records, provenance, source lineage, and temporal projection on foreign-key or injected failure', () => {
    ...
    expect(memories.getFact(asFactId('fact-a'))).toEqual(fact('fact-a')); expect(memories.getFact(asFactId('fact-b'))).toBeUndefined(); expect(db.prepare('SELECT COUNT(*) count FROM semantic_correction_records').get()).toEqual({ count: 0 }); expect(db.prepare('SELECT COUNT(*) count FROM worker_jobs').get()).toEqual({ count: 0 }); expect(db.prepare('PRAGMA foreign_key_check').all()).toEqual([])
  })

  it('rolls back the canonical correction when mandatory lexical repair enqueue fails', () => {
    const memories = new MemoryRepository(db); const corrections = new CorrectionRepository(db); memories.createFact(fact('fact-a'))
    db.exec("CREATE TRIGGER fail_correction_repair BEFORE INSERT ON worker_jobs BEGIN SELECT RAISE(ABORT,'forced repair failure'); END")
    expect(() => corrections.correct('correction-a', { previousFactId: asFactId('fact-a'), factId: asFactId('fact-b'), value: 'Tokyo', provenance: provenance('event-b'), effectiveAt: at(2), recordedAt: at(2) })).toThrowError(expect.objectContaining({ code: 'PERSISTENCE_FAILED' }))
    expect(memories.getFact(asFactId('fact-a'))).toEqual(fact('fact-a')); expect(memories.getFact(asFactId('fact-b'))).toBeUndefined(); expect(db.prepare('SELECT COUNT(*) count FROM semantic_correction_records').get()).toEqual({ count: 0 }); expect(db.prepare('SELECT COUNT(*) count FROM worker_jobs').get()).toEqual({ count: 0 })
  })
})
