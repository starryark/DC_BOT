...
export function replayDeletionObligations(database: DatabaseSync, obligations: readonly DeletionObligation[]): void {
  ...
  try {
    for (const obligation of obligations) {
      ...
      // Replayed obligations never passed through the tombstone-insert
      // trigger that purges lexical rows, so the same purge runs here; an
      // index row over a redacted, tombstoned, or emptied record must not
      // survive restore even though query-time checks would not serve it.
      database.prepare('DELETE FROM memory_search_latin WHERE target_table=? AND target_id=?').run(obligation.targetTable, obligation.targetId)
      database.prepare('DELETE FROM memory_search_cjk WHERE target_table=? AND target_id=?').run(obligation.targetTable, obligation.targetId)
    }
