...
import { ACTIVE_V1_VERSION, syntheticIdempotencyKey, syntheticSnowflake, syntheticTimestamp } from './dataset'
...
interface DriveResult {
  ...
  readonly operationCounts: { ingress: number, events: number, generations: number, deliveries: number }
...
async function driveScenario(run: EvaluationRuntimeRun, scenario: ScenarioSpec, options: RunOptions, redact: (kind: string, id: string) => string): Promise<DriveResult> {
  const v = ACTIVE_V1_VERSION
  const seed = options.seed
  const id = scenario.scenarioId
  const counts = { ingress: 0, events: 0, generations: 0, deliveries: 0 }
