...
export class ReconciliationQueue {
  ...
  claim(worker: string, now: string, leaseMs: number): ReconciliationJob | undefined {
    ...
    return new UnitOfWork(this.db).run((database) => {
      const claimed = database.prepare(`UPDATE worker_jobs SET status='leased',lease_owner=?,lease_expires_at=?,lease_token=?,attempt_count=attempt_count+1 WHERE job_id=(SELECT job_id FROM worker_jobs WHERE ((status='ready' AND available_at<=?) OR (status='leased' AND lease_expires_at<=?)) ORDER BY priority DESC,available_at,job_id LIMIT 1) RETURNING *`).get(worker, expires, token, now, now) as Row | undefined
