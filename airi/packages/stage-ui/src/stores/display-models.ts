...
/**
 * Manifest of locally packaged Live2D archives served from the app's public dir.
 *
 * Written by `packages/stage-ui-live2d/scripts/build-dori-zips.ts`. The folder is
 * gitignored because the archives are third-party assets, so a checkout without
 * it must behave exactly as before — hence the silent empty fallback.
 */
const externalLive2DPresetsManifestUrl = '/dori/index.json'

interface ExternalLive2DPresetEntry {
  id: string
  name: string
  url: string
}

async function loadExternalLive2DPresets(): Promise<DisplayModelURL[]> {
  try {
    const response = await fetch(externalLive2DPresetsManifestUrl)
    if (!response.ok)
      return []

    const entries = await response.json() as ExternalLive2DPresetEntry[]
    ...
    return entries
      ...
      .map(entry => ({
        ...
        // Deliberately older than the built-in presets so neither these nor the
        // built-ins outrank the user's own imports in the importedAt sort below.
        importedAt: 1733113886000,
      }))
  }
  catch {
    return []
...
export const useDisplayModelsStore = defineStore('display-models', () => {
  ...
  async function loadDisplayModelsFromIndexedDB() {
    ...
    const models: DisplayModel[] = [...displayModelsPresets, ...await loadExternalLive2DPresets()]
  ...
  async function resetDisplayModels() {
    ...
    // Locally packaged archives are install-provided, not user imports, so a
    // reset keeps them exactly as a fresh load would.
    displayModels.value = [...displayModelsPresets, ...await loadExternalLive2DPresets()]
