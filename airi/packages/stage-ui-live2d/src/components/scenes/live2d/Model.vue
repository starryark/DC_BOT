...
import { adaptInternalModel } from '../../../utils/model-adapter'
...
async function loadModel() {
  ...
  try {
    ...
    const motionGroupNames = Object.keys(motionManager.definitions)
    const detectedIdleGroup = motionGroupNames.find(group => group.toLowerCase() === 'idle')
      ?? motionGroupNames.find(group => /^idle\d*$/i.test(group))
