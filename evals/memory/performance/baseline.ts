...
import { parseCostEvidence } from './cost-evidence'
import { liveArtifactDigest } from './live-artifact'
...
function collectEligibilityReasons(run: LoadedRun, side: 'baseline' | 'candidate', reasons: Set<IncompatibilityReason>): void {
  ...
  // A warmup failure leaves the measured rows looking complete and clean, so a
  // run carrying one would otherwise still qualify as a reference. The warmups
  // are what put the workload into the state its measured samples were taken
  // from; a run whose warmups did not execute measured something else.
  if (derived.warmupFailures > 0)
    reasons.add(side === 'baseline' ? 'baseline-warmup-failed' : 'candidate-warmup-failed')
}
...
export function loadRun(
  ...
  const disagreements = [...summaryDisagreements(published, recomputed), ...costEvidenceDisagreements(published, manifest)]
...
function summaryDisagreements(published: Record<string, unknown>, recomputed: ReturnType<typeof deriveRunState>): readonly string[] {
  ...
  // NOTICE:
  // `warmupFailures` postdates the first published v2 runs, and an accepted
  // baseline directory written before it exists has no such key. Those runs
  // could not have carried a warmup finding either — the kind did not exist and
  // `parseRunFindingsJsonl` would reject one — so their recomputed count is
  // always 0 and defaulting the absent field to 0 keeps them loadable without
  // weakening the check for runs that do publish it.
  // Removal condition: when no accepted pre-`warmupFailures` baseline is in use.
  compare('warmupFailures', published.warmupFailures ?? 0, recomputed.warmupFailures)
...
/**
 * Cost fields a run's own bytes can settle without an external price document.
 *
 * No price document is available at load time, so this deliberately evaluates
 * no price: it checks only that `costAvailability` and `costEvidence` agree,
 * that the embedded live artifact still parses, that its recomputed canonical
 * digest is the one published, and that the run actually imported it. The
 * numeric recomputation belongs at the G8 boundary, where the approved price
 * document is supplied.
 *
 * Runs that predate cost derivation publish neither field and are unaffected.
 */
function costEvidenceDisagreements(published: Record<string, unknown>, manifest: RunManifest): readonly string[] {
  const available = published.costAvailability === 'available'
  const evidence = published.costEvidence
  if (evidence === undefined)
    return available ? ['costEvidence-missing'] : []
  if (!available)
    return ['costEvidence-without-availability']

  let parsed: ReturnType<typeof parseCostEvidence>
  try {
    parsed = parseCostEvidence(evidence)
  }
  catch {
    return ['costEvidence-invalid']
  }

  const disagreements: string[] = []
  if (liveArtifactDigest(parsed.liveArtifact) !== parsed.liveArtifactDigest)
    disagreements.push('costEvidence-artifact-digest')
  if (!manifest.importedLiveArtifactDigests.includes(parsed.liveArtifactDigest))
    disagreements.push('costEvidence-artifact-unimported')
  if (manifest.priceDocumentDigest !== parsed.priceDocumentDigest)
    disagreements.push('costEvidence-price-binding')
  return disagreements
}
