...
import { buildReport, computeNormalizedResultDigest, runIsValidForGate } from './report'
...
describe('report whole-run validity', () => {
  ...
  it('an applicable standard assertion failure invalidates the run', () => {
    const results = allPassing()
    const index = results.findIndex(r => r.scenarioId === 'CONTEXT-001')
    results[index] = makeResult({ scenarioId: 'CONTEXT-001', assertions: [{ assertionId: 'CONTEXT-001-A', passed: false, diagnostic: 'redacted:kind:0000000000000000' }] })
    expect(runIsValidForGate(build(results).summary)).toBe(false)
  })
...
describe('report measurement evaluation gate', () => {
  ...
  it('an approved threshold that the measurement satisfies is counted as passed and keeps the run valid', () => {
    ...
    const row = JSON.parse(report.scenarioJsonl.split('\n').find(line => line.includes('DELIV-001'))!) as ScenarioResult
    expect(row.measurements[0]!.evaluated).toBe(true)
    expect(runIsValidForGate(report.summary)).toBe(true)
  })
...
// An artifact verifier recomputes the digest a summary claims rather than
// trusting it. That only proves anything while the verifier and the report
// generator share one normalization implementation, so this pins them together.
describe('normalized result digest reuse', () => {
  it('recomputes the digest a report published for the same inputs', () => {
    const results = allPassing()
    const report = build(results)
    expect(computeNormalizedResultDigest(dataset, results, { datasetDigest: activeV1Digest(), seed: 20260802 }))
      .toBe(report.summary.normalizedResultDigest)
  })

  it('recomputes the same digest from results whose elapsed time was dropped', () => {
    const results = allPassing()
    const published = results.map(({ elapsedMs: _elapsedMs, ...rest }) => rest)
    expect(computeNormalizedResultDigest(dataset, published, { datasetDigest: activeV1Digest(), seed: 20260802 }))
      .toBe(build(results).summary.normalizedResultDigest)
  })

  it('produces a different digest when a scenario outcome changes', () => {
    const tampered = allPassing()
    tampered[0] = makeResult({ scenarioId: tampered[0]!.scenarioId, outcome: 'failed' })
    expect(computeNormalizedResultDigest(dataset, tampered, { datasetDigest: activeV1Digest(), seed: 20260802 }))
      .not
      .toBe(build(allPassing()).summary.normalizedResultDigest)
  })
})
