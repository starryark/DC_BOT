...
import { MemoryError, RETRIEVAL_STAGE_ORDER } from '@proj-airi/memory-domain'
...
const retrievalCorpusItemSchema = v.strictObject({
  itemId: v.pipe(v.string(), v.regex(/^[a-z0-9-]+$/)),
  role: v.picklist(['relevant', 'irrelevant', 'unauthorized_relevant', 'lifecycle_invalid', 'temporal_before', 'temporal_after']),
  content: v.pipe(v.string(), v.minLength(1), v.maxLength(280)),
  offsetSeconds: v.optional(v.pipe(v.number(), v.integer())),
})

const retrievalSpecSchema = v.strictObject({
  ...
  requestedModes: v.pipe(v.array(v.string()), v.minLength(1)),
  corpus: v.pipe(v.array(retrievalCorpusItemSchema), v.minLength(1)),
  judgments: v.pipe(v.array(v.strictObject({
    itemId: v.pipe(v.string(), v.regex(/^[a-z0-9-]+$/)),
    relevance: v.picklist([0, 1]),
  })), v.minLength(1)),
  expectations: v.strictObject({
    authorization: v.picklist(['authorized_only']),
    lifecycle: v.picklist(['current_only']),
    temporal: v.picklist(['inclusive']),
  }),
})

export type RetrievalSpec = v.InferOutput<typeof retrievalSpecSchema>
...
export const scenarioSchema = v.strictObject({
  ...
  /** Frozen retrieval declaration; required exactly for retrieval scenarios. */
  retrieval: v.optional(retrievalSpecSchema),
...
export function parseDataset(input: unknown): Dataset {
  ...
  for (const scenario of parsed.scenarios) {
    ...
    if (scenario.category === 'retrieval' && !scenario.retrieval)
      throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} is missing its frozen retrieval declaration`)
    if (scenario.category !== 'retrieval' && scenario.retrieval)
      throw new MemoryError('INVALID_PAYLOAD', `non-retrieval scenario ${scenario.scenarioId} declares retrieval data`)
    if (scenario.retrieval) {
      const itemIds = new Set<string>()
      for (const item of scenario.retrieval.corpus) {
        if (itemIds.has(item.itemId))
          throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} has duplicate corpus item ${item.itemId}`)
        itemIds.add(item.itemId)
      }
      const judgedIds = new Set<string>()
      for (const judgment of scenario.retrieval.judgments) {
        if (!itemIds.has(judgment.itemId))
          throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} judgment references missing corpus item ${judgment.itemId}`)
        if (judgedIds.has(judgment.itemId))
          throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} has duplicate judgment ${judgment.itemId}`)
        judgedIds.add(judgment.itemId)
      }
      if (judgedIds.size !== itemIds.size)
        throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} does not judge every corpus item`)
      for (const mode of scenario.retrieval.requestedModes) {
        if (!RETRIEVAL_STAGE_ORDER.includes(mode as never))
          throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} requests unknown mode ${mode}`)
      }
      if (scenario.retrieval.cutoff > scenario.retrieval.corpus.length)
        throw new MemoryError('INVALID_PAYLOAD', `retrieval scenario ${scenario.scenarioId} cutoff exceeds corpus size`)
    }
...
export interface RetrievalMetrics {
  ...
  readonly relevantMissed: number
  readonly precisionAtCutoff: number
  readonly recallAtCutoff: number
...
export interface RetrievalQueryResult {
  ...
  readonly appliedModes: readonly string[]
  readonly rankedItems: readonly {
  ...
  readonly temporalViolations: readonly string[]
}
...
export interface ScenarioResult {
  ...
  readonly retrieval?: RetrievalQueryResult
