...
import { ACTIVE_V1_VERSION, activeV1Dataset, activeV1DatasetObject, activeV1Digest, DEFAULT_SEED, multilingualV1Dataset, multilingualV1DatasetObject, multilingualV1Digest, scenariosForSuite, SMOKE_SUITE_REQUIRED_IDS, syntheticSnowflake, syntheticTimestamp } from './dataset'
...
describe('contract taxonomy', () => {
  ...
  it('evaluator schema version is the frozen value', () => {
    expect(EVALUATOR_SCHEMA_VERSION).toBe(3)
  })
})

describe('frozen retrieval dataset', () => {
  it('is canonical, synthetic, and complete for RET-001 through RET-010', () => {
    const dataset = multilingualV1Dataset()
    expect(multilingualV1Digest()).toBe(sha256Canonical(multilingualV1DatasetObject()))
    expect(dataset.scenarios.map(scenario => scenario.scenarioId)).toEqual(Array.from({ length: 10 }, (_, index) => `RET-${String(index + 1).padStart(3, '0')}`))
    for (const scenario of dataset.scenarios) {
      expect(scenario.retrieval).toBeDefined()
      expect(scenario.retrieval!.requestedModes).toEqual(['lexical'])
      expect(scenario.retrieval!.judgments).toHaveLength(scenario.retrieval!.corpus.length)
    }
  })

  it('rejects a judgment that references a missing corpus item', () => {
    const bad = clone(multilingualV1DatasetObject()) as { scenarios: Array<{ retrieval: { judgments: Array<{ itemId: string }> } }> }
    bad.scenarios[0]!.retrieval.judgments[0]!.itemId = 'missing-item'
    expect(() => parseDataset(bad)).toThrow(/references missing corpus item/)
  })

  it('rejects duplicate corpus items and duplicate judgments', () => {
    const duplicateItem = clone(multilingualV1DatasetObject()) as { scenarios: Array<{ retrieval: { corpus: unknown[], judgments: unknown[] } }> }
    duplicateItem.scenarios[0]!.retrieval.corpus.push(clone(duplicateItem.scenarios[0]!.retrieval.corpus[0]))
    expect(() => parseDataset(duplicateItem)).toThrow(/duplicate corpus item/)

    const duplicateJudgment = clone(multilingualV1DatasetObject()) as { scenarios: Array<{ retrieval: { judgments: unknown[] } }> }
    duplicateJudgment.scenarios[0]!.retrieval.judgments.push(clone(duplicateJudgment.scenarios[0]!.retrieval.judgments[0]))
    expect(() => parseDataset(duplicateJudgment)).toThrow(/duplicate judgment/)
  })

  it('rejects unknown retrieval modes and incomplete judgments', () => {
    const unknownMode = clone(multilingualV1DatasetObject()) as { scenarios: Array<{ retrieval: { requestedModes: string[] } }> }
    unknownMode.scenarios[0]!.retrieval.requestedModes = ['invented']
    expect(() => parseDataset(unknownMode)).toThrow(/requests unknown mode/)

    const missingJudgment = clone(multilingualV1DatasetObject()) as { scenarios: Array<{ retrieval: { judgments: unknown[] } }> }
    missingJudgment.scenarios[0]!.retrieval.judgments.pop()
    expect(() => parseDataset(missingJudgment)).toThrow(/does not judge every corpus item/)
  })
})
