...
class TTS:
    ...
    def _get_ref_spec(self, ref_audio_path):
        # torchaudio 2.11 delegates file decoding to TorchCodec, which has no
        # Windows wheels. SoundFile is already a GPT-SoVITS dependency and
        # handles WAV paths (including Unicode paths) natively on Windows.
        raw_samples, raw_sr = sf.read(ref_audio_path, dtype="float32", always_2d=True)
        raw_audio = torch.from_numpy(raw_samples.T.copy())
