...
| IEV-601 | IMP-601 | Explicit user authorization unblocked IMP-601 prerequisite. Query contract (MemoryPort.searchMemory) and scope-based authorization are verified to exist. | User explicit instruction 2026-08-08. |
| IEV-606-001 | IMP-606 | FTS5 schema v9 adds `memory_search_latin` (unicode61 tokenizer) and `memory_search_cjk` (trigram tokenizer) virtual tables, hex-encoded `auth_scope`, and segment population triggers. Migration runner advances from any prior version. Schema checksum in `v9.ts`. | `airi/packages/memory-sqlite/src/schema/v9.ts`; `airi/packages/memory-sqlite/src/migrations/index.ts` |
| IEV-606-002 | IMP-606 | `capabilityForLexicalQuery` correctly routes CJK queries (JP/ZH/KO) to `fulltext_cjk` and Latin queries to `fulltext_latin`. `requireCapability` throws `UNSUPPORTED_CAPABILITY` if the backend does not advertise the required capability. `M1_SQLITE_CAPABILITIES` now includes both `fulltext_latin` and `fulltext_cjk`. All unit tests in `port.test.ts` pass. | `airi/packages/memory-domain/src/capabilities.ts`; `port.test.ts` |
| IEV-606-003 | IMP-606 | `SearchRepository.searchMemory` routes lexical queries to the correct FTS5 table, enforces auth-scope hex filter, and returns `MemoryHit[]` with ranked bm25 ordering. Repository integration tests (`search.test.ts`) pass against a real in-memory SQLite instance. | `airi/packages/memory-sqlite/src/repositories/search.ts`; `search.test.ts` |
| IEV-606-004 | IMP-606 | `multilingual-v1` evaluation suite: 10 retrieval scenarios (RET-001–RET-010) covering JP, ZH, KO, and EN queries. Evaluator ran to completion with `applicablePassed: 10`, `applicableTotal: 10`, `zeroToleranceFailures: 0`. Dataset digest `e0a13fe840fe04a4958e4167f0acf951e41a9b0a6cc8b8ee157ef4ba2f025105`. Result digest `87acfb90836103e8b75cead49e388536488ec2676cc836c6ead4396ec28016a2`. | `airi/services/discord-bot/evals/memory/datasets/multilingual-v1.json`; evaluator run 2026-08-08 |
| IEV-606-005 | IMP-606 | Smoke suite passed: `applicablePassed: 7`, `applicableTotal: 7`, `zeroToleranceFailures: 0`. Lint exits 0 (0 errors). All 1590 tests pass. `git diff --check` exits 0. Commit `97824112ff6c4233a0e334e7b8df1d62aafd4638`. | CI run 2026-08-08 |
